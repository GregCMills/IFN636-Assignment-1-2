...
describe('ClerkAuthAdapter (unit)', () => {
  let auth;
  let _savedAdapterCache;
  let _savedClerkCache;
  let _clerkStub;
  ...
  before(() => {
    ...
    _clerkStub = {
      ...
      requireAuth:     () => (req, res, next) => { req.auth = { userId: 'stubbed_user' }; next(); },
      clerkClient: {
        users: {
    ...
    auth = require('../services/auth/ClerkAuthAdapter');
  });
...
const AuthAdapter = require('../services/auth/AuthAdapter');
...
describe('AuthAdapter (base class)', () => {
  ...
  const syncMethods = ['contextMiddleware', 'requireAuth', 'adminOnly', 'getUserId'];
  const asyncMethods = ['getUser', 'getUsers'];

  syncMethods.forEach(method => {
    it(`${method}() throws 'Not implemented'`, () => {
      expect(() => adapter[method]()).to.throw('Not implemented');
    });
  });

  asyncMethods.forEach(method => {
    it(`${method}() throws 'Not implemented' (async)`, async () => {
      try {
        await adapter[method]('any');
