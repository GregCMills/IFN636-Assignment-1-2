...
describe('Profile Routes', () => {
  ...
  describe('GET /api/auth/profile', () => {
    ...
    it('should return the profile for an authenticated user', async () => {
      sinon.stub(User, 'findOne').resolves({ address: '123 Main St', phone: '0400000000' } as any);
      const res = await request(app).get('/api/auth/profile');
  ...
  describe('PUT /api/auth/profile', () => {
    ...
    it('should update and return the profile for an authenticated user', async () => {
      sinon.stub(User, 'findOneAndUpdate').resolves({ address: '456 New St', phone: '0411111111' } as any);
      const res = await request(app)
        .put('/api/auth/profile')
