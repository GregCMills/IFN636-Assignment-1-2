...
class ClerkAuthAdapter extends AuthAdapter {
   ...
   * request.  Called once in server.ts with `app.use()`.
   *
   ...
   * @returns {Promise<NormalisedUser>}
   ...
   * @returns {Promise<Record<string, NormalisedUser>>}
   */
   ...
   *
   * @param {any} clerkUser — raw Clerk user object from the API
