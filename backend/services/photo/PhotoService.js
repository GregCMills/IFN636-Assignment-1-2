 ...
 * handlers behind three clean methods.  Controllers only need to call
 * uploadPhoto() and deletePhoto() — they know nothing about sharp, multer
 * buffers, or filesystem paths.
 *
 * ## Singleton
 ...
 */
const PhotoProcessingDirector = require('./PhotoProcessingDirector');
const PhotoHandlerFactory      = require('./handlers/PhotoHandlerFactory');
...
class PhotoService {
   ...
   */
  async deletePhoto(entityType, entityId) {
    const handler = PhotoHandlerFactory.create(entityType);
   ...
   */
  async getPhotoUrl(entityType, entityId) {
    const handler = PhotoHandlerFactory.create(entityType);
